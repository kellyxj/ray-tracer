class SceneSelector {
    ...
    initShadows() {
        ...
        //scene.AAtype = antiAliasing.none;
        scene.sampleRate=2;
    ...
    initCSG() {
        ...
        cylinder2.rayTranslate(0,-1,0);
        
        var slab = new Slab();
        slab.rayTranslate(0,0,1.3);
        slab.rayRotate(85, 1, 0, 0);
        var intersect1 = new Intersection(cylinder2, slab);

        var slab2 = new Slab();
        slab2.rayRotate(-30, 1, 0, 0);
        slab2.rayTranslate(0,0,-1);
        var intersect2 = new Intersection(intersect1, slab2);
        intersect2.rayTranslate(12,0,1);

        intersect2.setMaterial(mirror);
        scene.items.push(intersect2);

        var xSlab = new Slab();
        xSlab.rayRotate(90, 1, 0, 0);
    ...
    initBlur() {
        ...
        var light = new Light(-3, 0, 1, .5);

        var rotateAnim = new AnimationRotate(30, 0, 0, 1);
        light.animations.push(rotateAnim);
        
        light.rayScale(.3,.3,.3);

        var translateAnim = new AnimationTranslate(15, 0, -1, 0);
        light.animations.push(rotateAnim);
        light.animations.push(translateAnim);
